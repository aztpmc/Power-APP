# ExportLC-02-ShipmentDeadlineReminder

Shipping after the latest shipment date is an incurable discrepancy, so this fires earlier and more often than the expiry reminder.
...
|---|---|
| `ShipmentDeadlineReminderDays` | Semicolon-separated day offsets, e.g. `30;14;7;3;1`. |
...
2. For each offset, queries `01_ExportLCs` for records whose `LatestShipmentDate` is exactly that many days away.
3. Builds an idempotency key `ExportLC-02-ShipmentDeadlineReminder:<record>:<offset>`.
...
2. Put a test LC's `LatestShipmentDate` exactly N days out, where N is one of the configured offsets.
