# ExportLC-07-ProceedsMaturityReminder

Chases accepted-but-unpaid proceeds before maturity, and again after it via FLOW-08's overdue sweep.
...
|---|---|
| `ProceedsMaturityReminderDays` | Semicolon-separated day offsets, e.g. `30;14;7;3;1`. |
...
2. For each offset, queries `07_Proceeds` for records whose `MaturityDate` is exactly that many days away.
3. Builds an idempotency key `ExportLC-07-ProceedsMaturityReminder:<record>:<offset>`.
...
2. Put a test LC's `MaturityDate` exactly N days out, where N is one of the configured offsets.
