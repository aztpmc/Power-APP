# ExportLC-01-LCExpiryReminder

Warns the responsible officer before a credit expires. Offsets come from 18_WorkflowConfig key LCExpiryReminderDays, so changing 30;14;7;3;1 changes the whole schedule with no redeployment.
...
|---|---|
| `LCExpiryReminderDays` | Semicolon-separated day offsets, e.g. `30;14;7;3;1`. |
...
2. For each offset, queries `01_ExportLCs` for records whose `ExpiryDate` is exactly that many days away.
3. Builds an idempotency key `ExportLC-01-LCExpiryReminder:<record>:<offset>`.
...
2. Put a test LC's `ExpiryDate` exactly N days out, where N is one of the configured offsets.
